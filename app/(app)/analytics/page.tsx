...
export default function AnalyticsPage() {
  ...
  const weakestSubjects = attemptedSubjects.filter(
    ...
    <main className="min-h-screen select-none bg-white px-5 py-5 text-[#0a1038] md:px-6">
      <div className="w-full space-y-4">
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative pb-4 text-[12px] font-medium transition-all duration-300 ${
        ...
        <footer className="flex min-h-10 items-center justify-between rounded-xl bg-violet-50 px-5 py-3 text-[12px] font-normal text-[#3b2b8f]">
          ...
          <div className="hidden items-center gap-2 font-medium md:flex">
...
function Overview({
  ...
  weakestSubjects: SubjectDiagnostic[]
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <section className="grid grid-cols-1 gap-3 xl:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr]">
        <div className="relative overflow-hidden rounded-2xl border border-[#d9d0ff] bg-gradient-to-br from-[#f1eaff] via-white to-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.05)]">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 text-[13px] font-medium text-[#10163f]">
              BLL Readiness Score
            ...
            <div className="mt-6 flex items-end justify-between gap-4">
              <div>
                <div className="text-[52px] font-medium leading-none tracking-[-0.06em] text-violet-700">
                  {currentScore}%
                </div>
                <div
      ...
      <section className="grid grid-cols-1 gap-3 xl:grid-cols-[1.05fr_1.05fr_0.85fr]">
        <GlassCard title="AI Executive Summary" badge="Lexora AI">
          {canUsePremiumAnalytics ? (
            <div>
              <h2 className="text-[18px] font-medium leading-[1.35] tracking-[-0.03em] text-[#0a1038]">
              ...
              <div className="mt-6 rounded-xl bg-violet-50 p-4">
                <div className="mb-3 text-[12px] font-medium text-[#11183d]">
        ...
        <GlassCard title="What’s Driving Your Score">
          <div className="grid grid-cols-2 gap-7">
            <div>
              <div className="mb-3 text-[13px] font-medium text-emerald-700">
                Helping Your Score
              </div>
            ...
            <div>
              <div className="mb-3 text-[13px] font-medium text-rose-700">
                Hurting Your Score
              </div>
          ...
          {primaryWeakArea ? (
            <div className="space-y-4">
              ...
              <button
                ...
                className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-violet-700 text-[13px] font-medium text-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:bg-violet-800"
              >
        ...
        <GlassCard title="7-Day Improvement Forecast">
          {canUseBLLAnalytics && chartData.some((point) => point.score > 0) ? (
            <div className="grid grid-cols-[1fr_190px] gap-4">
              ...
              <div className="space-y-4 pt-5 text-[12px] font-medium">
...
function rangeLabel(range: string) {
  ...
  if (range === "7d") return "May 12 — May 18, 2024"
  if (range === "14d") return "Last 14 days"
  if (range === "30d") return "May 12 — May 18, 2024"
    ...
    <section className="rounded-2xl border border-[#e2e7f1] bg-white/90 p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] backdrop-blur-md transition-all duration-300 hover:shadow-[0_12px_28px_rgba(15,23,42,0.06)]">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            {icon}
            <h2 className="text-[16px] font-medium tracking-[-0.02em] text-[#070c2d]">
              {title}
            </h2>
...
function KpiCard({
  ...
  return (
    <div className="rounded-2xl border border-[#e3e8f3] bg-white p-5 shadow-[0_8px_20px_rgba(15,23,42,0.035)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(15,23,42,0.06)]">
      ...
      <div className="text-[12px] font-medium text-[#11183d]">
        {title}
      </div>

      <div className="mt-2 text-[26px] font-medium tracking-[-0.04em] text-[#070c2d]">
        {value}
      </div>

      <div className={`mt-3 text-[12px] font-medium ${negative ? "text-rose-600" : "text-emerald-600"}`}>
...
function SummaryMini({
  ...
  return (
    <div className="border-r border-slate-100 last:border-r-0">
      ...
      <div className={`mt-3 text-[11px] font-medium ${toneClass}`}>
        {title}
      </div>
      <p className="mt-2 pr-3 text-[11px] font-normal leading-5 text-[#52617f]">
        {text}
      </p>
      <button type="button" className="mt-3 text-[11px] font-medium text-violet-700">
        View details →
      </button>
    </div>
  )
...
function SnapshotMetric({
  ...
  return (
    <div>
      ...
      <div className={`mt-1 text-[14px] font-medium ${danger ? "text-rose-600" : "text-[#10163f]"}`}>
        {value}
      </div>
    ...
    <div className="mb-4 flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <div className="text-[12px] font-medium leading-5 text-[#11183d]">
            {title}
          </div>
          <div className={`text-[12px] font-medium ${tone === "green" ? "text-emerald-600" : "text-rose-600"}`}>
            {value}
          </div>
        </div>
  ...
  return (
    <div className="rounded-xl bg-violet-50 p-4">
      <div className="flex items-start gap-4">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-violet-700 text-lg font-medium text-white">
          {step}
        </div>

        <div>
          <div className="text-[13px] font-medium leading-5 text-[#11183d]">
            {title}
          </div>

          <span className="mt-2 inline-flex rounded-md bg-violet-200 px-2 py-0.5 text-[11px] font-medium text-violet-700">
            {time}
          </span>
...
function RiskColumn({
  ...
  return (
    <div className={`rounded-xl border p-4 ${toneClass}`}>
      ...
      <div className="mt-3 min-h-[70px] space-y-2">
        {items.length === 0 ? (
          ...
          items.slice(0, 3).map((item) => (
            <div key={item.name} className="flex items-center justify-between gap-2 text-[12px] font-medium text-[#11183d]">
              <span>{item.name}</span>
      ...
      <button type="button" className="mt-3 h-9 w-full rounded-lg border border-current bg-white/50 text-[12px] font-medium">
        {button}
      </button>
    </div>
  )
  ...
  return (
    <div className="flex min-h-[220px] items-center justify-center rounded-xl border border-dashed border-violet-200 bg-violet-50 p-5 text-center">
      <div>
        ...
        <div className="mt-3 text-[13px] font-medium text-[#11183d]">Upgrade required</div>
...
function RangeBtn({
  ...
  return (
    <button
      ...
      className={`h-7 rounded-full px-3 text-[11px] font-medium transition-all duration-300 ${
        active
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-lg">
      <div className="text-xs font-medium text-[#11183d]">{label}</div>
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-lg">
      ...
      {payload.map((item: any) => (
        <div key={item.dataKey} className="mt-1 text-xs font-medium" style={{ color: item.color }}>
